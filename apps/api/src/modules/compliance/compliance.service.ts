...
export class ComplianceService {
  ...
  generateEInvoice(tenantId: string, invoiceData: Record<string, unknown>): EInvoiceResult {
  ...
  generateEWayBill(tenantId: string, billData: Record<string, unknown>): EWayBillResult {
    const ewbNo = `${Math.floor(100000000000 + Math.random() * 900000000000)}`;
    const validDays = (billData.distance as number ?? 100) > 200 ? 3 : 1;
  ...
  getGstrStatus(tenantId: string): GstrStatusResult {
    const now = new Date();
